    ...
    fun start() {
        ...
        try {
            when (tftpPacket) {
                is RRQPacket -> {
                    thread(start = true) {
                        sendFile(StandardCharsets.US_ASCII.decode(tftpPacket.fileName).toString(), curSocket, recievedPacket.socketAddress)
                    }
                }
                is WRQPacket -> {
                    thread(start = true) {
                        AckPacket(0).send(curSocket, recievedPacket.socketAddress)
                        getFile(curSocket, tftpPacket.fileName.toString())
                    }
                }
            }
